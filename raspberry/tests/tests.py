...
class MessageTest(TestCase):
    @Override(TestCase)
    def setUp(self):
        self.api = SerialAPI(production=False)

    def test_machine_code(self):
        expected = "0001000"
        output = self.api._convert_to_machine_code(0, 10)
        self.assertEqual(expected, output)

    def test_parse(self):
        dic = self.api._parse_function_status({"function": 0, "status": 200})

        self.assertEqual(dic.get(0, None), 200)

        dic = self.api._parse_sensor_readings(

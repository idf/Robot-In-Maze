...
class MessageTest(TestCase):
    ...
    def test_parse(self):
        ind, dic = self.commander._parse_function_status({"function": 0, "status": 200})
        self.assertEqual(ind, FUNCTION)
        self.assertEqual(dic.get(0, None), 200)

        ind, dic = self.commander._parse_sensor_readings(
            {"sensors": [{"sensor": 0, "value": 100}, {"sensor": 1, "value": 30}, {"sensor": 2, "value": 30}]})
        self.assertEqual(ind, SENSOR)
